...
class ProphetModel(Model):
    ...
    def __init__(self, series_params: SeriesParams,
        ...
        if not isinstance(idx, (pd.DatetimeIndex, pd.PeriodIndex)):
            raise ValueError("ProphetModel requires a DatetimeIndex or PeriodIndex as series index")
    ...
    def forecast(self, prediction: PredictParams) -> pd.DataFrame:
        future = self.model.make_future_dataframe(periods=prediction.horizon,
                                                  freq=prediction.freq if prediction.freq else "D")
...
class LSTMModel(Model):
    ...
    def __init__(self,
        ...
        self.logger.info("Try to fit model %s model params model_params %s", self.name, model_params)
    ...
    def _model(self):
        model = Sequential()
        model.add(LSTM(32, activation="tanh", kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4),
                       return_sequences=True, input_shape=(self.window, 1)))
        model.add(Dropout(0.2))
        model.add(LSTM(16, activation="tanh", kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4)
    ...
    def forecast(self, prediction: PredictParams) -> pd.DataFrame:
        ...
        if self.trend is not None:
            ...
            trend_future = np.arange(1, prediction.horizon + 1) * slope + float(self.trend.iloc[-1])
        else:
            trend_future = 0.0
...
def models_factory(name: str, series_params, model_params: ModelParams, logging_params: LoggingParams) -> Model:
    registry = {
...
def get_index(series: pd.Series, horizon: int) -> pd.Index:
    ...
    if isinstance(idx, pd.DatetimeIndex):
        ...
        if freq is not None:
            ...
            step = idx[-1] - idx[-2]
            future = [idx[-1] + step * i for i in range(1, horizon + 1)]
    ...
    if pd.api.types.is_numeric_dtype(idx):
        ...
        future = np.arange(idx[-1] + step, idx[-1] + step * (horizon + 1), step)
        return pd.Index(future)
