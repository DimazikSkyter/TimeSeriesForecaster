...
class ForecastParams:
    ...
    trend_smoothing: Optional[str] = None
...
#Переделать, отфильтровать только периоды меньше 1/3 длины и больше M (передать из параметров), использовать diff(),
    ...
    async def init_models(self, series_params: SeriesParams, predict_params: PredictParams) -> dict[str, Model]:
        async def worker(model_name: str) -> tuple[str, Optional[Model]]:
            model_params = ModelParams({**asdict(self.params), **asdict(predict_params)})
            try:
                model = await asyncio.to_thread(models_factory,
                                                model_name,
                                                series_params,
                                                model_params,
                                                self.logger_params)
        ...
        return {name: model for name, model in (t.result() for t in done) if model is not None}
    ...
    def predict(self, series: pd.Series, predict_params: PredictParams) -> pd.DataFrame:
        ...
        with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
            self.logger.debug("The final predict df\n###########################\n%s", df)
        return df
    ...
    def analyze_series(self, series: pd.Series) -> SeriesParams:
        ...
        periodogram_candidates: Set[int] = self._get_periodogram_candidates(series_ds,
                                                                            top_n=top_n,
                                                                            max_period=max_period,
                                                                            proximity=0.1)

        fixed_candidates: Set[int] = self.params.candidate_periods.copy() #todo пока исключили, подумать, как вернуть
        ...
        stl_with_period: List[Tuple[DecomposeResult, int]] = self._check_and_accept_candidates(series_ds, periodogram_candidates)
        ...
        if len(stl_with_period) > 1:
            ...
            trend = stl_with_period[0][0].trend
            resid = stl_with_period[0][0].resid
    ...
    @staticmethod
    def calc_r2(total_series, conditional_series) -> float:
    ...
    def _check_and_accept_candidates(self, series: pd.Series, candidates: Set[int]) -> List[Tuple[any, int]]:
        success_candidates = []
        ...
        return  [(ordered_candidates[0], ordered_candidates[1]) for ordered_candidates in sorted(success_candidates, key=lambda x: x[2])][:self.params.max_cycles_in_ts]
